...
async function runMigrations() {
  ...
  // Use service_role client for DDL. If unavailable, skip auto-migration silently.
  const adminClient = supabaseAdmin;
  if (!adminClient) {
    return { status: 'skipped', reason: 'no_service_role_key' };
  }

  try {
    // Bootstrap: ensure tracking table exists
    const { error: bootstrapErr } = await adminClient.rpc('exec_sql', {
      sql: `
    ...
    if (bootstrapErr && bootstrapErr.message?.includes('does not exist')) {
      return {
        status: 'needs_manual',
        reason: 'exec_sql function missing — run prod_bootstrap.sql in SQL Editor',
        sql: getFullSQL(),
      };
    }
    if (bootstrapErr) {
