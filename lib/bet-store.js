...
export function getFriendBalances(period) {
  ...
  FRIENDS.forEach(f => {
    ...
    const wonPayouts = friendBets
      .filter(b => b.status === 'won')
      .reduce((s, b) => s + (b.payout || 0), 0);
    // For demo: add some variance so leaderboard looks interesting
    const randomBonus = Math.floor(Math.random() * 500) * (f.id === ME_ID ? 0 : 1);
    balances[f.id] = STARTING_BALANCE - totalStaked + wonPayouts + randomBonus;
  });
